...
def createTable(request, source, destination):
    ...
    #dajax.script('$("#pln_gld").html("' + str(pln_gld)+'")')
    #dajax.append('#pln_gld', 'innerHTML', pln_gld)
    #dajax.prepend('#pln_gld', 'innerHTML', pln_gld)
    gld_pln=''
    for comm in Commission.objects.all():
    ...
    #dajax.script('$("#gld_pln").html("' + str(gld_pln)+'")')
