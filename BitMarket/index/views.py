...
def testNewCommission(request):
    ...
    GLD_wallet = UserWallet.objects.filter(user=user, cryptocurrency=Cryptocurrency.objects.filter(name='GLD')[0])[0]
    ...
    i=0
    for amount in amounts['BTC_GLD']:
        print (user.id-1+i)%UserProxy.objects.all().count()
        if (user.id-1+i)%UserProxy.objects.all().count()==0:
            print (user.id-1+i)%UserProxy.objects.all().count()
            print "qwertyuiop"
            user.newCommission(source_amount=amount[1], destination_amount=amount[0], source_wallet=BTC_wallet, destination_wallet=GLD_wallet, dead_line=now)
        i=i+1
    i=0
    for amount in amounts['GLD_BTC']:
        if (user.id-1+i)%UserProxy.objects.all().count()==0:
            user.newCommission(source_amount=amount[0], destination_amount=amount[1], source_wallet=GLD_wallet, destination_wallet=BTC_wallet, dead_line=now)
        i=i+1
...
def getBoughtHistory(request):
    for history in History.getBoughtHistory(Cryptocurrency.objects.filter(name='BTC')[0],Cryptocurrency.objects.filter(name='GLD')[0], sort='amount_sold'):
        print history
    return render_to_response('master/index.html', {'local': locals()})
def getExchangeHistory(request):
    for history in History.getExchangeHistory(Cryptocurrency.objects.filter(name='GLD')[0],Cryptocurrency.objects.filter(name='BTC')[0], sort='amount_sold'):
        print history
        print '1'
    return render_to_response('master/index.html', {'local': locals()})